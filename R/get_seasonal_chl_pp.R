...
win_stack <- raster::stack(paste0(raw.dir,"/",winter))
win_stack <- raster::clamp(win_stack, lower=chl.min, upper=chl.max, useValues=FALSE)
win_stack <- log(win_stack)
raster::writeRaster(win_stack, filename=file.path(save.dir, "winter_chl_log.nc"), format = "CDF", overwrite = TRUE)

spring <- list.files(raw.dir, pattern = "06-OCCCI")
spr_stack <- raster::stack(paste0(raw.dir,"/",spring))
spr_stack <- raster::clamp(spr_stack, lower=chl.min, upper=chl.max, useValues=FALSE)
spr_stack <- log(spr_stack)
raster::writeRaster(spr_stack, filename=file.path(save.dir, "spring_chl_log.nc"), format = "CDF", overwrite = TRUE)

summer <- list.files(raw.dir, pattern = "09-OCCCI")
sum_stack <- raster::stack(paste0(raw.dir,"/",summer))
sum_stack <- raster::clamp(sum_stack, lower=chl.min, upper=chl.max, useValues=FALSE)
sum_stack <- log(sum_stack)
raster::writeRaster(sum_stack, filename=file.path(save.dir, "summer_chl_log.nc"), format = "CDF", overwrite = TRUE)

fall <- list.files(raw.dir, pattern = "12-OCCCI")
fal_stack <- raster::stack(paste0(raw.dir,"/",fall))
fal_stack <- raster::clamp(fal_stack, lower=chl.min, upper=chl.max, useValues=FALSE)
fal_stack <- log(fal_stack)
raster::writeRaster(fal_stack, filename=file.path(save.dir, "fall_chl_log.nc"), format = "CDF", overwrite = TRUE)


#############################################################################################################
#################### Primary Production Log Tranform ########################################################
#############################################################################################################
...
win_stack <- raster::clamp(win_stack, lower=pp.min, upper=pp.max, useValues=FALSE)
win_stack <- log(win_stack)
raster::writeRaster(win_stack, filename=file.path(save.dir, "winter_pp_log.nc"), format = "CDF", overwrite = TRUE)

spring <- list.files(raw.dir, pattern = "06-OCCCI")
spr_stack <- raster::stack(paste0(raw.dir,"/",spring))
spr_stack <- raster::clamp(spr_stack, lower=pp.min, upper=pp.max, useValues=FALSE)
spr_stack <- log(spr_stack)
raster::writeRaster(spr_stack, filename=file.path(save.dir, "spring_pp_log.nc"), format = "CDF", overwrite = TRUE)

summer <- list.files(raw.dir, pattern = "09-OCCCI")
sum_stack <- raster::stack(paste0(raw.dir,"/",summer))
sum_stack <- raster::clamp(sum_stack, lower=pp.min, upper=pp.max, useValues=FALSE)
sum_stack <- log(sum_stack)
raster::writeRaster(sum_stack, filename=file.path(save.dir, "summer_pp_log.nc"), format = "CDF", overwrite = TRUE)

fall <- list.files(raw.dir, pattern = "12-OCCCI")
fal_stack <- raster::stack(paste0(raw.dir,"/",fall))
fal_stack <- raster::clamp(fal_stack, lower=pp.min, upper=pp.max, useValues=FALSE)
fal_stack <- log(fal_stack)
raster::writeRaster(fal_stack, filename=file.path(save.dir, "fall_pp_log.nc"), format = "CDF", overwrite = TRUE)
